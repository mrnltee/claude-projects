...
/**
 * Whether the vehicle checkboxes appear at all.
 *
 * Distinct from `shouldAskVehicleTypes`, and the difference is the point:
 *
 *  NPLV   the field is a QUESTION. Motorcycle and sedan are pre-ticked
 *         because that is what the band names, and the reporter is
 *         expected to correct it for their street.
 *  NPATV  the field is a STATEMENT. Every type is ticked, because that is
 *         what "not passable to all types of vehicles" means. It is shown
 *         rather than hidden so the reporter can see the six claims their
 *         report is about to carry — and can still untick one if their
 *         street genuinely behaves differently.
 *  PATV   nothing is stopped, so there is nothing to show.
 *
 * Hiding it at NPATV, which is what the form did first, wrote those six
 * claims silently. Showing a pre-filled statement is the honest version of
 * the same shortcut.
 */
export function shouldShowVehicleTypes(code: FloodDepthCode | null): boolean {
  if (!code) return false;
  const band = getMmdaAssessment(code)?.band;
  return band === "NPLV" || band === "NPATV";
}
