...
describe("when to ask about vehicle types", () => {
  ...
  it("does not ask where nothing passes — the answer is entailed, not open", () => {
    // Note this is about the PROMPT, not visibility: at NPATV the field is
    // still rendered (see shouldShowVehicleTypes) with every type ticked.
    // It just stops being phrased as a question.
...
describe("when to show the vehicle checkboxes", () => {
  it("shows them from Binti upward — the first NPLV depth", () => {
    expect(shouldShowVehicleTypes("BINTI")).toBe(true);
    expect(shouldShowVehicleTypes("TUHOD")).toBe(true);
  });

  it("THE CORRECTION: keeps showing them at Hita and beyond", () => {
    // The form first hid this field at NPATV and wrote all six vehicle
    // types into the report behind a one-line disclosure. Pre-filling is
    // fine; doing it out of sight is not. The reporter sees the six
    // claims and can amend them.
    for (const code of ["HITA", "BAYWANG", "HINDI_MADAANAN"] as const) {
      expect(shouldShowVehicleTypes(code)).toBe(true);
    }
  });

  it("hides them at Bukong-bukong and below, where nothing is stopped", () => {
    for (const code of ["WALANG_BAHA", "GUTTER_DEEP", "BUKONG_BUKONG"] as const) {
      expect(shouldShowVehicleTypes(code)).toBe(false);
    }
    expect(shouldShowVehicleTypes(null)).toBe(false);
  });

  it("is shown wherever a default would be recorded, and nowhere else", () => {
    // The invariant that matters: a report never carries vehicle types the
    // reporter could not see, and never shows an empty field to fill in
    // that we would ignore.
    for (const o of DEPTH_SCALE) {
      expect(shouldShowVehicleTypes(o.code)).toBe(getDefaultVehicleTypes(o.code).length > 0);
    }
  });

  it("asking is a subset of showing", () => {
    for (const o of DEPTH_SCALE) {
      if (shouldAskVehicleTypes(o.code)) expect(shouldShowVehicleTypes(o.code)).toBe(true);
    }
  });
});
