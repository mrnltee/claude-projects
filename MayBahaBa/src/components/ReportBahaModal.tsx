...
export function ReportBahaModal({ open, onClose, onSuccess, initialLocation }: ReportBahaModalProps) {
  ...
  /**
   * Both derived from the chosen depth rather than stored, so neither can
   * disagree with the picker.
   *
   * `showVehicleTypes` — whether the field exists at all (NPLV and NPATV).
   * `askVehicleTypes`  — whether it is a question (NPLV) or a pre-filled
   *                      statement the reporter can amend (NPATV).
   */
  const showVehicleTypes = shouldShowVehicleTypes(floodDepth);
    ...
    <Modal open={open} onClose={handleClose} titleId={titleId} title="Report Baha">
      {submitState.kind === "success" ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/*
            No "Add optional details" toggle. The depth decides whether
            this field means anything, so the depth — not a second tap —
            decides whether it appears:

              PATV   (Bukong-bukong and below)  absent entirely
              NPLV   (Binti, Tuhod)             a question, light vehicles pre-ticked
              NPATV  (Hita and above)           a statement, all types ticked

            Both bands render the same checkboxes. Only the prompt above
            them changes, because at NPATV the reporter is confirming what
            the band already entails rather than telling us something new.
          */}
          {showVehicleTypes && (
            <fieldset data-testid="vehicle-types">
              ...
              <p className="mt-0.5 text-xs text-(--color-ink-muted)">
                {askVehicleTypes
                  ? "Sa lalim na ito, may sasakyang nakakadaan at may hindi — kaya malaking tulong kung alam natin kung alin."
                  : "Sa lalim na ito, walang nakakadaan ayon sa MMDA — kaya nakatala na lahat. Puwede mong alisin ang alinman kung iba ang nakita mo."}
              </p>
              <div className="mt-2 flex flex-wrap gap-2">
